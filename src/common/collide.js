...
export const collideGroup = (group, obj) =>
  group.map((gi) => isCollide(obj, gi)).some(Boolean);

const isCollideX = (objects, player) =>
  collideGroup(objects, nextObject(moveByDir(player, 'x')));
const isCollideY = (objects, player) =>
  collideGroup(objects, nextObject(moveByDir(player, 'y')));
